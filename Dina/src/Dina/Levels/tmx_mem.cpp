...
void set_alloc_functions() {
	...
	if (!tmx_img_load_func) tmx_img_load_func = Dina::Graphic::LoadVoidTexture;
}
...
resource_holder* pack_tileset_resource(tmx_tileset *value) {
	resource_holder *res = static_cast<resource_holder*>(node_alloc(sizeof(resource_holder)));
	if (res) {
		res->type = RC_TSX;
...
void resource_deallocator(void *val, const char *key UNUSED) {
	...
	if (val) {
		...
		if (rc_holder->type == RC_TSX)
			...
			free_template(rc_holder->resource._template);
		tmx_free_func(val);
	}
}
