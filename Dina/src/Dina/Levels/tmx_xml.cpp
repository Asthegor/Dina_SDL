...
static int parse_object(xmlTextReaderPtr reader, tmx_object *obj, int is_on_map, tmx_resource_manager *rc_mgr, const char *filename) {
	...
	if ((value = (char*) xmlTextReaderGetAttribute(reader, (xmlChar*) "id")))
	{ /* id */
		obj->id = atoi(value);
		tmx_free_func(value);
	} else if (is_on_map) {
	...
	if ((value = (char*) xmlTextReaderGetAttribute(reader, (xmlChar*) "template"))) { /* template */
		if (rc_mgr) {
			...
			if (tmpl && tmpl->type == RC_TX) {
				obj->template_ref = tmpl->resource._template;
			}
		}
		if (!(obj->template_ref)) {
...
static int parse_tileset_list(xmlTextReaderPtr reader, tmx_tileset_list **ts_headadr, tmx_resource_manager *rc_mgr, const char *filename) {
	...
	*ts_headadr = res_list;
	...
	if ((value = (char*)xmlTextReaderGetAttribute(reader, (xmlChar*)"source"))) { /* source */
		...
		ret = parse_tileset(sub_reader, res, rc_mgr, ab_path.c_str()); /* and parses the tsx file */
...
static int parse_template(xmlTextReaderPtr reader, tmx_template *_template, tmx_resource_manager *rc_mgr, const char *filename) {
	char *name;
	int curr_depth;
	...
	do {
		...
		if (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT) {
			...
			if (!strcmp(name, "tileset")) {
				parse_tileset_list(reader, &(_template->tileset_ref), rc_mgr, filename);
			} else if (!strcmp(name, "object")) {
				if (!parse_object(reader, _template->object, 0, rc_mgr, filename)) return 0;
			} else {
				/* Unknow element, skip its tree */
				if (xmlTextReaderNext(reader) != 1) return 0;
			}
