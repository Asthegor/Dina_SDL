...
int mk_map_tile_array(tmx_map *map) {
	...
	if (!(map->tiles = static_cast<tmx_tile**>(tmx_alloc_func(NULL, map->tilecount * sizeof(void*))))) {
		tmx_errno = E_ALLOC;
		return 0;
	}
...
/* duplicate a string */
const char* tmx_strdup(const char *str) {
	std::string res { str };
...
const char* mk_absolute_path(const char *base_path, const char *rel_path) {
...
void* load_image(void **ptr, const char *base_path, const char *rel_path) {
	if (tmx_img_load_func) {
		std::string ap_img = mk_absolute_path(base_path, rel_path);
		if (ap_img.empty()) return 0;
		*ptr = tmx_img_load_func(ap_img.c_str());
